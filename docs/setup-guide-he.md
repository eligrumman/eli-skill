...
מדריך מלא מאפס לסטאפ ש-**eli** מקים בשבילכם: מכונה always-on — שרת ענן זול, או Mac ישן (MacBook / Mac Mini) שכבר יש לכם — שמריצה צי של אייג'נטים של Claude Code, נגישה מכל מקום דרך Tailscale, ונשלטת מטלפון דרך Termux או Termius. בלי בוט, בלי דמון מותאם — רק SSH ו-Claude Code.
...
המכונה ה-always-on שלכם, יחד עם הטלפון שממנו שולטים בה, על tailnet אחד:

| מכשיר | תפקיד |
|---|---|
| **המכונה ה-always-on שלכם — בחרו אחת:** | |
| • **שרת בענן** (Hetzner) | מריץ את האייג'נטים 24/7. אין מה להשאיר טעון בבית. |
| • **Mac ישן בבית** (MacBook / Mac Mini) | אותה עבודה, על חומרה שכבר יש לכם. השאירו אותו מחובר לחשמל וער. |
| **טלפון** (Termux / Termius) | השלט רחוק שלכם — מתחברים ב-SSH מכל מקום דרך Tailscale. |
